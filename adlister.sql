...
-- 1. Email query
select title, description, category, email from ads join users u on ads.users_id = u.id;

-- 2. Category query
select distinct title, description, category from ads join users u on ads.users_id = u.id;
